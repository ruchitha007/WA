...
**In addition to the `submission.txt` file, you must submit a short video tour of your photo app and the stories you implemented. The video length should be no longer than 12 × k seconds, where k is the number of story points you completed. A typical video length is 2 minutes or less.** This should be a simple screen recording with audio. On macOS you can use QuickTime and on Windows you can use VLC to take screen recordings. **Please do not spend too much time creating the video. You will not be graded on production quality.**
